...
| 5 | **After Sept 17: trigger the deploys the credit pause skipped** | **alot.land first** — Netlify → alotofland → Deploys → *Trigger deploy*. That publishes the paired lot packages, the team section, the stamps and the price toggle, all committed but sitting undeployed. Then Actions → *Deploy sub-apps* → Run workflow, once for `time-audit` and once for `mission-control`; their production branches moved and the first build on each was skipped. | David | todo |
...
| Netlify credit burn fixed — sub-apps off main (158 wasted deploys/cycle), video refresh conditional (27) | 3 |
| Call menu on 615-538-6878 — press 1 → realtor (roll to David), silence → David; voicemail on both | 3 |
