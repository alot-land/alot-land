...
| 5 | **Kick one deploy each on time-audit and mission-control** | Their production branches moved to `deploy/<app>` and the first build on each was skipped by the credit pause. Actions → *Deploy sub-apps* → Run workflow, once for `time-audit` and once for `mission-control`. After that they only deploy when their folder changes. | David | todo |
