...
const MANUAL = [
  ...
  },
  {
    ...
    entries: [
      ...
      },
      {
        ...
        b: 'Above the unit mix is a rent estimator with two ladders. FREE: the blended Zillow ZORI rent for the ZIP, reshaped to each unit type\'s bedroom count using HUD Small Area FMR ratios — SAFMR levels are policy figures (roughly a 40th percentile), so only the RATIOS between bedroom counts are used, never the level itself. That turns one blended number into a real 1BR/2BR/3BR spread for $0. PAID: an address-level RentCast estimate with a comp-backed range, fetched only when you click "Address-level" and cached forever afterwards, because the free tier is 50 requests a month. The app counts every request it sends and shows "N/50 used this month" beside the button; at 50 the button switches off and will not fire again unless you tick a box confirming you want to spend past the free tier — which is billed to the card on your RentCast account. That tick resets whenever you leave the page, so it can never be left on by accident. Cache hits are free and never counted. One honest caveat: the counter runs on calendar months, and if your RentCast billing period starts on a different day of the month the two can drift apart — the dashboard is the authority. Neither writes into your unit mix until you press Apply. Rent is the single largest source of error in every downstream figure — NOI, cap rate, DSCR, verdict, max offer — so this is the input most worth improving.',
      },
      {
  ...
  },
  {
    ...
    entries: [
      ...
      {
        t: 'CHECK INPUTS — when the app refuses to give a verdict',
        b: 'A score computed from inputs that cannot describe a real building is not a calculation, it is a bad recommendation. So the engine now gates every verdict on a plausibility check, and where it fails you get an amber CHECK INPUTS instead of a grade — on the deal page, the off-market list, and the on-market list, each explaining exactly what looks wrong. Four things trip it: a price under $20,000 per unit (the anchor is a land-only or partial-parcel assessment, not a building), a price over $2,000,000 per unit (the unit count is understated — usually a county range like "25 - 99 UNITS" stored as its low end), a cap rate above 25% (the income and the price cannot both be right), and a rent above $20,000 per unit per month (a whole-building figure being applied to every unit). The thresholds sit far outside any real deal, so an unusually good one is never suppressed — only an impossible one. The numbers are still shown so you can see WHAT is wrong, and filtering by "Check inputs" pulls up every affected row. A real example this caught: a Glendale parcel published as 25-99 units with a $137,900 assessed value scored PURSUE 100/100 at a 172% cap rate. It now refuses to score at all.',
      },
      {
